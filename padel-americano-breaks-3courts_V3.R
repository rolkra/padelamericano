...
num_rounds <- 6
total_players <- 12
pause_groups <- list(9:12, 5:8, 1:4, 9:12, 5:8, 1:4)

MAX_LOCAL_STEPS <- 50000  # Wie viele Tausch-Versuche wir pro Start zulassen
MAX_ROUNDS <- 100
...
get_active_players <- function(total_players, pause_groups, num_rounds)  {
  ...
  for (r in 1:num_rounds) {
    active_players[[r]] <- setdiff(1:total_players, pause_groups[[r]])
  }
  active_players
}
...
while (best_score > 0 && rounds <= MAX_ROUNDS) {
  ...
  for (r in 1:num_rounds) {
    current_schedule[[r]] <- sample(setup$active_players[[r]])
  }
